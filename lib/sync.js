...
function synchronize(options) {
  ...
  function sha1(data) {
    const integrity = ssri.fromData(data, {algorithms: ['sha1']});
    return integrity.sha1[0].hexDigest();
  }

  function sha512(data) {
    const integrity = ssri.fromData(data, {algorithms: ['sha512']});
    return integrity.sha512[0].toString();
  }
          ...
          .flatMap(distribution => {
            if (tarballExists(distribution)) {
              if (distribution.binary) {
                const data = fs.readFileSync(packageTarballFilePath(distribution.name, distribution.version))
              ...
              .flatMap(data => {
                if (distribution.dist.integrity) {
                  if (sha512(data) != distribution.dist.integrity) {
                    return Bacon.later(0, new Bacon.Error('Integrity check with SHA512 failed for ' + distribution.name + '@' + distribution.version));
                  }
                } else if (sha1(data) != distribution.dist.shasum) {
                  return Bacon.later(0, new Bacon.Error('Integrity check with SHA1 failed for ' + distribution.name + '@' + distribution.version));
                }
                if (distribution.binary) {
  ...
  const manifest = require(path.resolve(options.manifest))
  const dependencies = dependenciesToArray(_.merge(manifest.dependencies, manifest.devDependencies));
