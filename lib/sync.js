  ...
  function resolveVersionAndDependencies(package) {
    return fetchMetadata(package.name)
      .map(metadata => {
          ...
          const version = semver.maxSatisfying( available, package.versionRange, true );

          if ( version === null ) {
              console.log( 'Available versions: ' + JSON.stringify( available, null, 2 ) );
              console.log( `Version range: ${package.versionRange}` );
              throw new Error( package.name + ': Suitable version not found' );
          }
